...
parser.add_argument("-dataset_code_dir", required=True,
                    help="Path to load the code in the dataset")

parser.add_argument("-dataset_nl_dir", required=True,
                    help="Path to load the nl in the dataset")

parser.add_argument("-dataset_AST_dir", required=True,
                    help="Path to AST files")
...
class Multi_model(nn.Module):
    ...
    def forward(self, enc_outputs, ast_outputs, src_embed, ast_embed, match_rela):
        ...
        fus, fus_self_attn =self.enc_self_attn(ast_embed1, src_embed, src_embed)

        enc_outputs = fus + enc_outputs
        return enc_outputs
...
def greedy_decoder(model, enc_input, ast_outputs, ast_embed, match_rela, start_symbol):
    ...
    #     print("dec_input的形状为：",dec_input.shape)
    #     dec_outputs, _, _, _ = model.decoder1(dec_input, enc_input, enc_outputs, ast_outputs, src_embed, ast_embed, match_rela)
    #     projected = model.projection(dec_outputs)
    #     prob = projected.squeeze(0).max(dim=-1, keepdim=False)[1]
...
if __name__ == '__main__':
    ...
    c1, c2 = generate_code_datasets(opt.dataset_code_dir)
    n1, n2, n3 = generate_nl_datasets(opt.dataset_nl_dir)
    sentences = generate_sentences(opt.dataset_code_dir, opt.dataset_nl_dir)

    src_vocab = make_code_vocab(opt.dataset_code_dir)
    # print("11111",src_vocab)
    src_vocab_size = len(src_vocab)

    tgt_vocab = make_nl_vocab(opt.dataset_nl_dir)
    ...
    max_ast_node = 100  # number of nodes of AST
    ...
    list_tok_AST = match_Tok_AST2(opt.AST_num, opt.dataset_AST_dir, opt.dataset_code_dir)
    ...
    for i1 in range(opt.AST_num):
        ...
        features1 = get_embed(opt.dataset_AST_dir + file_name,max_ast_node)
        adj1 = get_A(opt.dataset_AST_dir + file_name,max_ast_node)
        X.append(features1)
        Adj.append(adj1)
    ...
    for x, adj, inputs, _, _ in test_data_loader:
        ...
        for j in range(len(inputs)):
            ...
            print(sentences[tar_order][0], '->', y1)
            ...
            str3_line = []
            str2_line = str2.split()
            for line in str2_line:
                if line == 'EOS':
                    str3_line.append(line)
                    break
                else:
                    str3_line.append(line)
            str3 = " ".join(str3_line)
